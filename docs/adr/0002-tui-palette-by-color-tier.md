...
所有面板框线统一为单一中性结构色 `border`（真彩色=深蓝灰、16 色=Gray、单色=Reset），不按数据类别着色——类别已由面板标题色与数据色（保持鲜艳）承载，按类别着色只会重复编码并占用有限的配色预算。唯一例外是聚焦的 IP 面板以 `coral` 框线标示交互焦点（非类别语义）。色深档位设计为可注入，使现有 `TestBackend` 测试能钉死真彩色档以保持断言不变，并补充 16 色与单色档的快照测试。
